...
{
    public class UpdateTapCommandValidator : AbstractValidator<UpdateTapCommand>
    {
        private readonly IApplicationDbContext _context;

        public UpdateTapCommandValidator(IApplicationDbContext context)
        {
            _context = context;

            RuleFor(t => t.Id)
                .NotEmpty().WithMessage("Tap must not be null.")
                .MustAsync(TapExist).WithMessage("Tap does not exist.");
            ...
            RuleFor(t => t.CardId)
                .NotEmpty().WithMessage("CardI must not be null.")
        ...
        public async Task<bool> TapExist(int id, CancellationToken cancellationToken)
        {
