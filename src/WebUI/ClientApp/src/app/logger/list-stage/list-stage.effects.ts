import * as stage from './list-stage.actions';
...
export class StageEffects {
    constructor(private actions$: Actions, private stageClient: StagesClient) { }

    loadStages$ = createEffect(() =>
        this.actions$.pipe(
            ofType(stage.LOAD_STAGES_REQUEST),
            mergeMap(() => {
                return this.stageClient.getAll().pipe(
                    map(stages => stage.LOAD_STAGES_SUCCESS({stages})),
                    catchError(error => of(stage.LOAD_STAGES_FAIL({ error }))));
            })
        )
    );
}
